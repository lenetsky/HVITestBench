...
def main():
    ...
    try:
        ...
        if module_id < 0:
            raise Exception(f'Error opening module in chassis: {chassis_number}, {slot_number}, opened with ID: {module_id}')
        ...
        # TODO - ACTIVE LOW does not seem to work for front panel and ACTIVE HIGH does not seem to work for pxi lines
        ...
        instruction1 = sequence.programming.add_instruction('TriggerOn', 100, hvi.instructions.instructions_trigger_write.id)    # Add trigger write instruction to the sequence
        instruction1.set_parameter(hvi.instructions.instructions_trigger_write.trigger, sequence_trigger)            # Specify which trigger is going to be used
        ...
        instruction1.set_parameter(hvi.instructions.instructions_trigger_write.value, pyhvi.TriggerValue.ON)         # Specify trigger value that is going to be applied

        instruction2 = sequence.programming.add_instruction('TriggerOff', 1000, hvi.instructions.instructions_trigger_write.id)  # Add trigger write instruction to the sequence
        instruction2.set_parameter(hvi.instructions.instructions_trigger_write.trigger, sequence_trigger)            # Specify which trigger is going to be used
        ...
        instruction2.set_parameter(hvi.instructions.instructions_trigger_write.value, pyhvi.TriggerValue.OFF)        # Specify trigger value that is going to be applied
